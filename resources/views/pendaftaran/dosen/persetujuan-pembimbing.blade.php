...
<div class="btn-group scrollable-btn-group col-md-12">
 ...
 <a href="/pendaftaran/kp-skripsi/persetujuan"  class="btn bg-light border  border-bottom-0" style="border-top-left-radius: 40%;" >Persetujuan</a>
    @endif
  @endif
...
<a href="/pendaftaran/pembimbing/skripsi"  class="btn bg-light border  border-bottom-0 " style="border-top-right-radius: 40%;"  >Skripsi</a>
</div>
...
<div></div>
        <tr>        
            ...
            <a href="/usulan/detail/pembimbingprodi/{{($kp->id)}}"class="badge bg-success rounded-pill p-2 fas fa-eye" > Lihat Detail</a>
            </td>
            @endif
            @if ($kp->status_kp == 'SURAT PERUSAHAAN' || $kp->status_kp == 'KP DISETUJUI' )
            <td class="text-center">
            <a href="/suratperusahaan/detail/pembimbingprodi/{{($kp->id)}}"class="badge bg-success rounded-pill p-2 fas fa-eye" > Lihat Detail</a>
            </td>
            @endif

            @if ($kp->status_kp == 'DAFTAR SEMINAR KP' || $kp->status_kp == 'SEMINAR KP DIJADWALKAN' || $kp->status_kp == 'SEMINAR KP SELESAI')
            <td class="text-center">
            <a href="/daftar-semkp/detail/pembimbing/ {{($kp->id)}}" class="badge bg-success rounded-pill p-2 fas fa-eye" > Lihat Detail</a>
            </td>
            @endif

            @if ($kp->status_kp == 'BUKTI PENYERAHAN LAPORAN' || $kp->status_kp == 'KP SELESAI')
            <td class="text-center">
            <a href="/kpti10/detail/pembimbingprodi/{{($kp->id)}}"class="badge bg-success rounded-pill p-2 fas fa-eye" > Lihat Detail</a>
            </td>
            @endif 
...
<div></div>
        <tr>        
            ...
            <td class="text-center">{{$skripsi->keterangan}}</td> 
             ...
             <!-- PEMBIMBING -->
              <!-- USUL JUDUL-->
            @if ($skripsi->status_skripsi == 'USULAN JUDUL' || $skripsi->status_skripsi == 'USULAN JUDUL DISETUJUI KOORDINATOR SKRIPSI' || $skripsi->status_skripsi == 'USULAN JUDUL DISETUJUI PEMBIMBING 1' || $skripsi->status_skripsi == 'USULAN JUDUL DISETUJUI PEMBIMBING 2' ||$skripsi->status_skripsi == 'USULAN JUDUL DISETUJUI'  )
            <td class="text-center">
            <a href="/usuljudul/detail/pembimbing/{{($skripsi->id)}}" class="badge bg-success rounded-pill p-2 fas fa-eye" >  Lihat Detail</a>
            </td>
            @endif

             <!-- DAFTAR SEMPRO -->
             @if ($skripsi->status_skripsi == 'DAFTAR SEMPRO' || $skripsi->status_skripsi == 'DAFTAR SEMPRO DISETUJUI KOORDINATOR SKRIPSI' || $skripsi->status_skripsi == 'DAFTAR SEMPRO DISETUJUI PEMBIMBING' || $skripsi->status_skripsi == 'DAFTAR SEMPRO DISETUJUI PEMBIMBING 1' || $skripsi->status_skripsi == 'DAFTAR SEMPRO DISETUJUI PEMBIMBING 2' || $skripsi->status_skripsi == 'SEMPRO DISETUJUI' || $skripsi->status_skripsi == 'SEMPRO DIJADWALKAN'||$skripsi->status_skripsi == 'SEMPRO SELESAI') 
            <td class="text-center">
            <a href="/daftar-sempro/detail/pembimbing/{{($skripsi->id)}}" class="badge bg-success rounded-pill p-2 fas fa-eye" >  Lihat Detail</a>
            </td>
            @endif
             <!-- DAFTAR SIDANG -->
             @if ($skripsi->status_skripsi == 'DAFTAR SIDANG' || $skripsi->status_skripsi == 'DAFTAR SIDANG DISETUJUI KOORDINATOR SKRIPSI' || $skripsi->status_skripsi == 'DAFTAR SIDANG DISETUJUI PEMBIMBING' || $skripsi->status_skripsi == 'DAFTAR SIDANG DISETUJUI PEMBIMBING 1' || $skripsi->status_skripsi == 'DAFTAR SIDANG DISETUJUI PEMBIMBING 2' || $skripsi->status_skripsi == 'SIDANG DISETUJUI') 
            <td class="text-center">
            <a href="/daftar-sidang/detail/pembimbing/{{($skripsi->id)}}" class="badge bg-success rounded-pill p-2 fas fa-eye" >  Lihat Detail</a>
            </td>
            @endif
