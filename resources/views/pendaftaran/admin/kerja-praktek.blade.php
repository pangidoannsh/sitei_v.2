  ...
  <div></div>
        <tr>        
           ...
           @if ( $kp->status_kp == 'SURAT PERUSAHAAN DITOLAK' || $kp->status_kp == 'DAFTAR SEMINAR KP DITOLAK' || $kp->status_kp == 'DAFTAR SEMINAR KP ULANG' || $kp->status_kp == 'BUKTI PENYERAHAN LAPORAN DITOLAK' )           
            ...
            <td class="text-center p-2"> <small> Tanggal Usulan: <br></small>{{Carbon::parse($kp->tgl_created_usulan)->translatedFormat('l, d F Y')}}</td>
            @endif

             @if ($kp->status_kp == 'USULAN KP DITERIMA')           
            <td class="text-center p-2"> <small> Tanggal Diterima: <br></small>{{Carbon::parse($kp->tgl_disetujui_usulankp_kaprodi)->translatedFormat('l, d F Y')}}</td>
            @endif

             @if ($kp->status_kp == 'SURAT PERUSAHAAN' || $kp->status_kp == 'SURAT PERUSAHAAN DITOLAK')           
            <td class="text-center p-2"> <small>Tanggal Usulan: <br></small>{{Carbon::parse($kp->tgl_created_balasan)->translatedFormat('l, d F Y')}}</td>
            @endif

             @if ($kp->status_kp == 'KP DISETUJUI')           
            <td class="text-center p-2"> <small>Tanggal Disetujui: <br></small>{{Carbon::parse($kp->tgl_disetujui_balasan)->translatedFormat('l, d F Y')}}</td>
            @endif

            @if ($kp->status_kp == 'DAFTAR SEMINAR KP' || $kp->status_kp == 'DAFTAR SEMINAR KP DITOLAK' || $kp->status_kp == 'DAFTAR SEMINAR KP ULANG')           
            <td class="text-center p-2"> <small>Tanggal Usulan: <br></small>{{Carbon::parse($kp->tgl_created_semkp)->translatedFormat('l, d F Y')}}</td>
            @endif
            @if ($kp->status_kp == 'DAFTAR SEMINAR KP DISETUJUI')           
            <td class="text-center p-2"> <small>Tanggal Disetujui: <br></small>{{Carbon::parse($kp->tgl_created_semkp_kaprodi)->translatedFormat('l, d F Y')}}</td>
            @endif
            @if ($kp->status_kp == 'SEMINAR KP DIJADWALKAN')           
            <td class="text-center p-2"> <small>Tanggal Dijadwalkan: <br></small>{{Carbon::parse($kp->tgl_dijadwalkan)->translatedFormat('l, d F Y')}}</td>
            @endif
            @if ($kp->status_kp == 'SEMINAR KP SELESAI')           
            <td class="text-center p-2"> <small>Tanggal Selesai: <br></small>{{Carbon::parse($kp->tgl_selesai_semkp)->translatedFormat('l, d F Y')}}</td>
            @endif
            @if ($kp->status_kp == 'BUKTI PENYERAHAN LAPORAN' || $kp->status_kp == 'BUKTI PENYERAHAN LAPORAN DITOLAK')           
            <td class="text-center p-2"> <small>Tanggal Usulan: <br></small>{{Carbon::parse($kp->tgl_created_kpti10)->translatedFormat('l, d F Y')}}</td>
            @endif
            ...
            @elseif($kp->keterangan == 'Menunggu persetujuan Admin Prodi' && Auth::guard('web')->user()->role_id == 2 || $kp->keterangan == 'Menunggu persetujuan Admin Prodi' && Auth::guard('web')->user()->role_id == 3 || $kp->keterangan == 'Menunggu persetujuan Admin Prodi' && Auth::guard('web')->user()->role_id == 4 )
            ...
            @if ($kp->status_kp == 'DAFTAR SEMINAR KP' || $kp->status_kp == 'SEMINAR KP DIJADWALKAN' || $kp->status_kp == 'SEMINAR KP SELESAI' || $kp->status_kp == 'DAFTAR SEMINAR KP DITOLAK' || $kp->status_kp == 'DAFTAR SEMINAR KP ULANG' || $kp->status_kp == 'DAFTAR SEMINAR KP DISETUJUI')
            <td class="text-center p-2">
