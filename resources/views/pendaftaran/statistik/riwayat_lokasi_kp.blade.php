...
@section('content')
    ...
    <div class="container card p-4">
        ...
        <ol class="breadcrumb col-lg-12">
            ...
            <span class="px-2">|</span>
            <li><a href="/statistik/riwayat-kp" class="breadcrumb-item active fw-bold text-success px-1">Riwayat KP</a></li>
            <span class="px-2">|</span>
        ...
        @php
            ...
            foreach ($pendaftaran_kps as $riwayat_lokasi_kp) {
                $prodi_list[] = $riwayat_lokasi_kp->prodi->nama_prodi;
            }
        ...
        <table class="table table-responsive-lg rounded table-bordered table-striped" width="100%" id="datatablesjudulskripsiterdaftar">
                <thead class="table-dark">
                    <tr>
                        ...
                        <th class="text-center" scope="col">Perusahaan</th>
                        <th class="text-center" scope="col">Alamat</th>
                        <th class="text-center" scope="col">Bidang Usaha</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    @foreach ($pendaftaran_kps as $riwayat_lokasi_kp)
                        <div></div>
                        <tr>
                            <td class="text-center">{{ $riwayat_lokasi_kp->mahasiswa->nim }}</td>
                            <td class="text-left">{{ $riwayat_lokasi_kp->mahasiswa->nama }}</td>
                            <td class="text-center">{{ $riwayat_lokasi_kp->prodi->nama_prodi }}</td>
                            <td class="text-left">{{ $riwayat_lokasi_kp->nama_perusahaan }}</td>
                            <td class="text-left">{{ $riwayat_lokasi_kp->alamat_perusahaan }}</td>
                            <td class="text-left">{{ $riwayat_lokasi_kp->bidang_usaha }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
