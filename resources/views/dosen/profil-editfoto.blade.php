    ...
    <div class="col-lg-5">
        <form action="/profil-dosen/editfotodsn/{{ $dosen->id }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                @if ($dosen->gambar)
                    <img src="{{ asset('storage/' . $dosen->gambar) }}"
                        class="img-preview-dsn img-fluid mb-3 col-sm-5 d-block">
                @else
                    <img class="img-preview-dsn img-fluid mb-3 col-sm-5">
                @endif
                <input class="form-control @error('gambar') is-invalid @enderror" type="file" id="imgdosen"
                    name="gambar" onchange="previewImgdosen()">
                @error('gambar')
