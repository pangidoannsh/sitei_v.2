...
<div class="col-lg-5">
    <form action="/form-skripsi/edit/{{$skripsi->id}}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="nim" class="form-control @error('nim') is-invalid @enderror" value="{{ old('nim', $skripsi->nim) }}">
            @error('nim')
        ...
        <div class="mb-3">
            ...
            <input type="text" name="nama" class="form-control @error('nama') is-invalid @enderror" value="{{ old('nama', $skripsi->nama) }}">
            @error('nama')
              <div class="invalid-feedback">
                  {{$message}}
              </div>
        ...
        <div class="mb-3">
            ...
            <select name="prodi_id" class="form-select @error('prodi_id') is-invalid @enderror">
                ...
                @foreach ($prodis as $prodi)
                    <option value="{{$prodi->id}}" {{old('prodi_id', $skripsi->prodi_id) == $prodi->id ? 'selected' : null}}>{{$prodi->nama_prodi}}</option>
                @endforeach
            </select>
            @error('prodi_id')
            <div class="invalid-feedback">
        ...
        <div class="mb-3">
            ...
            <select name="konsentrasi_id" class="form-select @error('konsentrasi_id') is-invalid @enderror">
                ...
                @foreach ($konsentrasis as $konsentrasi)
                    <option value="{{$konsentrasi->id}}" {{old('konsentrasi_id', $skripsi->konsentrasi_id) == $konsentrasi->id ? 'selected' : null}}>{{$konsentrasi->nama_konsentrasi}}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="number" name="angkatan" class="form-control @error('angkatan') is-invalid @enderror" value="{{ old('angkatan', $skripsi->angkatan) }}">
            @error('angkatan')
              <div class="invalid-feedback">
                  {{$message}}
              </div>
