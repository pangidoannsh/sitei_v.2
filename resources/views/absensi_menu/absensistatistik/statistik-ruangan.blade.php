    ...
    <div class="container card p-4">
        <ol class="breadcrumb col-lg-12">
            @if (Auth::user()->role_id == 1 || Auth::user()->role_id == 2 || Auth::user()->role_id == 3 || Auth::user()->role_id == 4)
                <li><a href="{{ route('absensistatistikadmin') }}" class="px-1">Statistik Absensi (<span id="absensiCount"></span>)</a></li> 
            @else
                <li><a href="{{ route('absensistatistik') }}" class="px-1">Statistik Absensi (<span id="absensiCount"></span>)</a></li> 
            @endif
            <span class="px-2">|</span>
            <li><a href="{{ route('statistik-ruangan') }}" class="breadcrumb-item fw-bold text-success px-1">Ruangan (<span id="ruanganCount"></span>)</a></li>   
        </ol>
<table class="table table-responsive-lg text-center table-bordered table-striped" style="width:100%" id="datatables">
            ...
            <tbody>
                ...
                <tr >
                    ...
                    <td class="{{ $data['status'] == 'Sedang Digunakan' ? 'bg-success' : '' }}">{{ $data['status'] }}</td> <!-- Status ruangan -->
                </tr>
                @endforeach
            </tbody>
        </table>
