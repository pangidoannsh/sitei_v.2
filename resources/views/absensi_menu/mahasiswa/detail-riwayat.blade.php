        ...
        <div class="container card p-4">
            <div class="row absensi mt-4 mb-2">
                <div class="col-6 pl-2">
                    ...
                    <h6><span>Semester</span> {{ $detailabsensimahasiswa->first()->class->semester->semester ?? '-' }}
                        {{ $detailabsensimahasiswa->first()->class->semester->tahun_ajaran ?? '-' }}</h6>
                </div>
                <div class="col-6 ">
                    <h6><span>Program Studi</span> {{ $detailabsensimahasiswa->first()->class->prodi->nama_prodi ?? '-' }}
                    </h6>
                ...
                <tbody>
                    @foreach ($detailabsensimahasiswa as $index => $absensi)
                        <tr>
                            ...
                            <td>{{ $absensi->class->ruangan->nama_ruangan ?? '-' }}</td>
                            <td>{{ $absensi->perkuliahan->materi ?? '-' }}</td>
                            <td
                                class="@if ($absensi->keterangan === 'Sakit') bg-danger text-white @elseif($absensi->keterangan === 'Izin') bg-warning text-dark @elseif($absensi->keterangan === 'Hadir') bg-info text-white @else bg-secondary text-dark @endif"">
