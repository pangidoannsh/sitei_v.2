    ...
    <div class="col-sm-5">
        ...
        <div class="form-group row justify-content-center">
            ...
            <select name="gedung_id" class="form-select @error('gedung_id') is-invalid @enderror">
                ...
                <option value="{{ $gedung->id }}" {{ old('gedung_id') == $gedung->id ? 'selected' : null }}>{{ $gedung->nama_gedung }}</option>
                @endforeach
            </select>
            @error('gedung_id')
        ...
        <div class="form-group row justify-content-center">
            ...
            <div class="col-sm-7">
                <input name="nama_ruangan" class="form-control @error('nama_ruangan') is-invalid @enderror" rows="2" placeholder="Ruangan">{{ old('nama_ruangan') }}</input>
            @error('nama_ruangan')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
