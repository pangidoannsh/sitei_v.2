                    ...
                    <tr>
                        ...
                        <td class="text-start" style='white-space: nowrap'>
                            @if ($nextPertemuans[$abs->id] == 'Masuk')
                                <a href="{{ route('showQrCode', ['id' => session('idPerkuliahan')]) }}"
                                    class="badge bg-info border-0 text-center" style="border-radius:20px; padding:7px;">
                                    Masuk
                                </a>
                            @elseif ($nextPertemuans[$abs->id] && $nextPertemuans[$abs->id] <= 16)
                                {{ $abs->nomor_pertemuan }} <!-- Perbaikan disini -->
                                <button type="button" class="badge bg-primary border-0 text-center openAbsensiModal"
                                    style="border-radius:20px; padding:7px;" data-toggle="modal"
                                    data-target="#openAbsensiModal{{ $abs->id }}"
                            ...
                            <!-- Modal -->
                            @php
                                $daringCount = $abs->perkuliahan->where('jenis_perkuliahan', 'Daring')->count();
                            @endphp
                            <div class="modal fade" id="openAbsensiModal{{ $abs->id }}" tabindex="-1"
                                role="dialog" aria-labelledby="openAbsensiModalLabel{{ $abs->id }}"
                                aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header bg-dark">
                                            <h5 class="modal-title" id="openAbsensiModalLabel{{ $abs->id }}">Mulai
                                                Perkuliahan</h5>
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <form action="{{ route('buka_kelas') }}" method="POST">
                                                ...
                                                <input type="hidden" name="next_pertemuan"
                                                    value="{{ $nextPertemuans[$abs->id] }}">
                                                <!-- Gunakan nilai next_pertemuan dari $nextPertemuans -->
                                                <div class="form-group">
                                                    <label for="pertemuan">Pertemuan</label>
                                                    <input type="text" class="form-control" id="pertemuan"
                                                        name="pertemuan" value="{{ $nextPertemuans[$abs->id] }}"
                                                        readonly>
                                                </div>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control" id="jenis_perkuliahan"
                                                        name="jenis_perkuliahan" style="border: none; margin-left:-9px">
                                                        <option value="Luring">Luring</option>
                                                        <option value="Daring"
                                                            @if ($daringCount >= 8) disabled @endif>Daring
                                                        </option>
                                                    </select>
                                                </div>
...
@push('scripts')
    <script>
        ...
        $(document).ready(function() {
            $('#jenis_perkuliahan').on('change', function() {
                var daringCount = $(this).data('daring-count');
                if ($(this).val() === 'Daring' && daringCount >= 8) {
                    alert('Kelas daring sudah mencapai batas maksimal 8 kali.');
