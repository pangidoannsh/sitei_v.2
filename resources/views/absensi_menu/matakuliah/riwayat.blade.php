...
<div class="container card p-4">
    ...
    <ol class="breadcrumb col-lg-12">
        <li><a href="/matakuliah" class="px-1">Mata Kuliah ()</a></li> 
        <span class="px-2">|</span>
        <li><a href="{{ route('riwayat') }}" class="breadcrumb-item  px-1 fw-bold text-success">Riwayat ({{ $count }})</a></li>
        ...
        <span class="px-2">|</span>
        <li><a href="{{ route('ruangan-absensi-admin') }}" class="breadcrumb-item  px-1">Ruangan ()</a></li>                        
        @endif
    </ol>
@php
            ...
            <div class="d-none d-md-flex justify-content-between mb-3 filter">
                <div class="d-flex align-items-center">
                    <div class="dataTables_length input-group" style="width: max-content;">
                        ...
                        <select id="lengthMenuMatakuliahProdi" class="custom-select custom-select-md rounded-3 py-1" style="width: 55px;">
                   ...
                   @if (Auth::user()->role_id == 1 || Auth::user()->role_id == 5)
                    <div class="input-group ml-3" style="width: max-content;">
                        ...
                        <select id="prodiFilterMatakuliahProdi" class="custom-select custom-select-md rounded-3 py-1 text-capitalize" style="width: 83px;">
                   ...
                   @if (Auth::user()->role_id == 1 || Auth::user()->role_id == 5)
                    <div class="input-group ml-3" style="width: max-content;">
                        ...
                        <select id="semesterFilterRiwayatMatakuliahKajur" class="custom-select custom-select-md rounded-3 py-1 text-capitalize" style="width: 83px;">
                            <option value="" selected>Semua</option>
                            @foreach ($semester_list as $semester)
                    ...
                    <div class="input-group ml-3" style="width: max-content;">
                        ...
                        <select id="semesterFilterRiwayatMatakuliah" class="custom-select custom-select-md rounded-3 py-1 text-capitalize" style="width: 83px;">
                            <option value="" selected>Semua</option>
                            @foreach ($semester_list as $semester)
                                <option value="{{ $semester }}" class="text-capitalize">{{ $semester }}</option>
                            @endforeach
                ...
                <div class="dataTables_filter input-group" style="width: max-content;">
                    ...
                    <input type="search" class="form-control form-control-md rounded-3 py-1" id="searchFilterMatakuliahSkripsiProdi" placeholder="">
                </div>
            </div>
            ...
            <div class="d-flex flex-wrap justify-content-center gap-3 mb-3 filter d-block d-md-none">
                ...
                <div class="input-group mt-3" style="width: max-content;">
                    ...
                    <select id="prodiFilterMobileMatakuliahProdi" class="custom-select custom-select-md rounded-3 py-1 text-capitalize" style="width: 83px;">
                        <option value="" selected>Semua</option>
                        @foreach ($prodi_list as $prodi)
                            <option value="{{ $prodi }}" class="text-capitalize">{{ $prodi }}</option>
                        @endforeach
                ...
                <div class="input-group mt-3" style="width: max-content;">
                    ...
                    <select id="semesterFilterMobileProdiKajur" class="custom-select custom-select-md rounded-3 py-1 text-capitalize" style="width: 83px;">
                        <option value="" selected>Semua</option>
                        @foreach ($semester_list as $semester)
                            <option value="{{ $semester }}" class="text-capitalize">{{ $semester }}</option>
                        @endforeach
                ...
                <div class="dataTables_filter input-group mt-3" style="width: max-content;">
                    ...
                    <input type="search" class="form-control form-control-md rounded-3 py-1" style="width: 83px; id="searchFilterMobileMatakuliahProdi" placeholder="">
                </div>
            </div>
<table class="table table-responsive-lg text-center table-bordered table-striped" style="width:100%" id="datatablesMataKuliah">
  ...
  <tbody>
    @foreach ($matakuliah as $riwayat)
        <tr>
          ...
          @endif
          @if ($riwayat->dosenmatkul == !null)          
          <td>{{$riwayat->dosenmatkul->nama_singkat}}</td>
          @endif
          @if ($riwayat->dosenmatkul2 !== 0 && $riwayat->dosenmatkul2)
              <td>{{ $riwayat->dosenmatkul2->nama_singkat }}</td>
          @else
              <td>-</td>
          @endif          
          <td>{{$riwayat->hari}}</td>          
          ...
          <td class=" text-center" style='white-space: nowrap'>        
            <a href="{{ route('detail.statistik', ['matakuliah_id' => $mtk->id]) }}" class="badge bg-info p-1 mb-1" data-bs-toggle="tooltip" title="Lihat Detail"><i class="fas fa-info-circle" aria-hidden="true"></i></a>
            <a href="{{ route('download_pdf', ['matakuliah_id' => $riwayat->id])}}" class="badge bg-success p-1 mb-1" data-bs-toggle="tooltip" title="Lihat Detail"><i class="fas fa-download" aria-hidden="true"></i></a>
          </td>
          @else 
          @endif
        </tr>
