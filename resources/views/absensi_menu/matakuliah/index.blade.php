...
<div class="container card p-4">
    ...
    <ol class="breadcrumb col-lg-12">
      ...
      <span class="px-2">|</span>
        <li><a href="{{ route('riwayat') }}" class="breadcrumb-item  px-1">Riwayat ()</a></li>   
...
<table class="table table-responsive-lg text-center table-bordered table-striped" width="100%" id="datatablesMataKuliah">
  <thead class="table-dark">
    <tr>
      ...
      @if (Auth::user()->role_id == 1 || Auth::user()->role_id == 2 || Auth::user()->role_id == 3 || Auth::user()->role_id == 4 || Auth::user()->role_id == 5 || Auth::user()->role_id == 6 || Auth::user()->role_id == 7 || Auth::user()->role_id == 8)
  ...
  <tbody>
    @foreach ($matakuliah as $mtk)
        <tr>
          ...
          @endif
          <td>{{$mtk->sks}}</td>      
          @if ($mtk->semester == !null)          
          <td>{{$mtk->semester->semester}} {{$mtk->semester->tahun_ajaran}}</td>
          @endif
          @if ($mtk->dosenmatkul == !null)          
          <td>{{$mtk->dosenmatkul->nama_singkat}}</td>
          @endif
          @if ($mtk->dosenmatkul2 !== 0 && $mtk->dosenmatkul2)
              <td>{{ $mtk->dosenmatkul2->nama_singkat }}</td>
          @else
              <td>-</td>
          @endif
          <td>{{$mtk->hari}}</td>          
          ...
          <td class=" text-start" style='white-space: nowrap'>        
            <a href="/matakuliah/edit/{{$mtk->id}}" class="badge bg-warning mb-1" data-bs-toggle="tooltip"><i class="fas fa-pen"></i></a>   
            ...
            <button type="button" class="badge bg-danger border-0 text-center modalDeleted d-inline" value="{{$mtk->id}}">
                <i class="fas fa-trash-can"></i>
            </button>
            <a href="{{ route('download_pdf', ['matakuliah_id' => $mtk->id])}}" class="badge bg-success p-1 mb-1" data-bs-toggle="tooltip" title="Lihat Detail"><i class="fas fa-download" aria-hidden="true"></i></a>
          </td>
          @elseif (Auth::user()->role_id == 1 || Auth::user()->role_id == 5 || Auth::user()->role_id == 6 || Auth::user()->role_id == 7 ||Auth::user()->role_id == 8  ) 
